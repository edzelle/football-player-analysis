...
plt.vlines(ymin=0, ymax=200, x=8, colors='red')
...
players_and_clusters = np.transpose([player_names, P])

import csv
index = 0
with open('WR-cluster-output-no-college-8-encoded.csv', 'w') as myfile:
    for name, cluster in players_and_clusters:
        myfile.write(name +"," + cluster + ",\n")
